...
function setupTextAnimations() {
  ...
  if (headerContainer) {
    ...
    if (subtitle) {
      const split = new SplitText(subtitle, { type: "chars" });
      const chars = split.chars;
  ...
  // Team Names (Split Text Effect)
  gsap.utils.toArray("#team .card-name").forEach((name) => {
    new SplitText(name, { type: "chars" });
  });
...
function setupScrollAnimations() {
  ...
  gsap.set("#team .card-role, #team .card-desc, #team .card-socials a", {
    opacity: 0,
  });
      ...
      batch.forEach((wrapper, i) => {
        ...
        gsap.to(wrapper, {
          ...
          onStart: () => {
            ...
            // Target the divs created by SplitText (was previously spans)
            const nameChars = wrapper.querySelectorAll(".card-name > div, .card-name span");
  ...
  heroItems.forEach((item) => {
    ...
    item.addEventListener("mouseenter", () => {
      ...
      const cardElements = card
        ? card.querySelectorAll(".card-header, .sc-name, .sc-achievements li")
        : [];
